...
SELECT_AD_URL = os.getenv(
    "SELECT_AD_URL",
    f"http://{SERVER_IP}:5000/api/advertisements/select",
)
REPORT_AD_URL = os.getenv(
    "REPORT_AD_URL",
    f"http://{SERVER_IP}:5000/api/ad-play-logs/report",
)
...
class ViewerTrack:
    ...
    def summary(self, session_end_ts=None):
        ...
        watch_duration = max(0.0, end_ts - self.first_seen_ts)
        return {
            "viewer_id": self.track_id,
            "estimated_age": None if estimated_age is None else round(estimated_age, 1),
            "gender": gender,
    ...
    try:
        while True:
            ...
            if sensor_state == "Dark":
                if state == "ad" and selected_ad is not None and ad_start_ts is not None:
            ...
            if state == "ad":
                if not ret:
                    ...
                    display_frame, detections = collect_viewer_detections(frame, frame_index, gaze_state, annotate=False)
                ...
                if ad_capture is not None and (last_ad_frame_ts is None or now_ts - last_ad_frame_ts >= 0.03):
                    ...
                    if ok:
                        display_frame = cv2.resize(ad_frame, (FRAME_WIDTH, FRAME_HEIGHT))
                ...
                if selected_ad is not None and ad_start_ts is not None:
                    ...
                    if (not ok) or elapsed >= selected_ad["duration_seconds"]:
                        finalize_ad_session(selected_ad, ad_manager, ad_start_ts, now_ts)
                        state = "idle"
                        selection_start_ts = None
                        selection_request_sent = False
                ...
                if selected_ad is not None:
                    cv2.putText(
                        display_frame,
                        f"Playing ad {selected_ad['ad_id']} ({selected_ad['duration_seconds']:.1f}s)",
                        (20, 30),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 255, 255),
            ...
            if selection_request is None:
                ...
                if selection_request.error is not None:
                    ...
                    try:
                        ...
                        ad_path = resolve_media_path(selected_ad["media_filename"])
                        ad_capture = cv2.VideoCapture(ad_path)
                        if not ad_capture.isOpened():
                            raise RuntimeError(f"Cannot open ad media: {ad_path}")
                        ad_manager.reset()
                        state = "ad"
                        ad_start_ts = now_ts
                        last_ad_frame_ts = None
            ...
            remaining = max(0.0, AUDIENCE_WINDOW_SECONDS - (now_ts - selection_start_ts))
            cv2.putText(
                display_frame,
